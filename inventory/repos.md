...
| handbook | github | active | public | public-ok | yes | no | yes | yes | no | Hub-repo (voorheen Westmarch, hernoemd 2026-07-12): specs, inventaris, mkdocs-pipeline, sessie-startpunt (AGENTS.md-mandaat). Site importeert zichzelf niet (handbook_import blijft no); docs/ wel leesbaar via handbook-mcp (hub-exceptie in code, mcp-hub-self-read, 2026-07-22). Agent-registry: docs/agents/ is de canonieke bron per agent-rol (chat-facet + executie-facet per definitie); consumenten (ratatoskr-listener, habitat-seeds) lezen daar en kopieren niet. Bewaakt door scripts/check_agent_tools.py (naam/npub/facetten/tools/model + seed-kruising). |
